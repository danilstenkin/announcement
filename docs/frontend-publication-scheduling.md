...
## 0. Доступ (фронт ходит через BFF, не напрямую)

- **Базовый путь шлюза:** `/api/forteguide/v1/announce/...`
  Пример: `POST /api/forteguide/v1/announce/auto-announce/tickets/{id}/confirm-date`.
  (Ниже в документе пути даны в коротком виде `/auto-announce/...` и `/announcements/...` —
  это хвост после `/api/forteguide/v1/announce`.)
- **Авторизация:** JWT (`Authorization: Bearer <token>`). Заголовки `X-User-Id/Name/Email`
  фронту слать **не нужно** — их подставляет BFF из токена. Время везде — ISO-8601
  **с таймзоной** (Asia/Almaty, `+05:00`); отправляй и парси с оффсетом.
- **Права:** все ручки управления тикетами/публикациями требуют `manage_announcements`
  (или суперадмина). Без права — `403`.

> Прямые пути announce-сервиса (для бэкенда/отладки): сервис смонтирован под `root_path=/announce`,
> внутренние пути — `/auto-announce/...` и `/announcements/...`. Фронт это не использует — только шлюз выше.
